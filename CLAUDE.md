...
- **A task is a title, not a document** (v0.33.0, AIR-086). A benchmark of agents handing work across sessions showed they read task titles and status and never the 25-line body, and that the month/week slots only put a prerequisite (`upsert_month`) in front of `create_task`, which agents then skipped. So: no months, no week, no prd; `content` is one line of at most 200 characters for the one fact the code cannot give; `get_status` is the whole resume (overview + open tasks) so a session starts with one call. Time is `meta.completed_at`; the dashboard groups by it if it needs a calendar.
