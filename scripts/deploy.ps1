...
.DESCRIPTION
  Runs ON the server, from its checkout. Stops the "FronyBoard Server" scheduled task
  (uv sync cannot replace a running fronyboard.exe), optionally checks out -Tag and
...
# 1. stop — wait for the task to end, then kill any fronyboard process it left behind
schtasks /End /TN $TaskName | Out-Null
$procs = { Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like "*fronyboard*serve*" -or $_.Name -eq "fronyboard.exe" } }
